    ...
    <body>
        <?php
			...
			if(empty($_GET['date'])){
				...
				$result = $db -> query("select count(*) as total from access_info where instr(time, '{$_GET['date']}')");
				$result2 = $db -> query("select * from access_info where instr(time, '{$_GET['date']}')");
				$para = "&date={$_GET['date']}";
				$localdate = $_GET['date'];
			}
            ...
            echo '<p style="font-size:18px;">' . $localdate . '&nbsp;访问量：' . $today_num .'次&nbsp;&nbsp;网络爬虫或是机器人访问：' . $today_robot . '次</p>';
            echo "<p style=\"font-size:18px;\">" . $localdate . '&nbsp;搜索引擎跳转：' . "谷歌：{$seo['google']}次&nbsp;&nbsp;百度：{$seo['baidu']}次&nbsp;&nbsp;360：{$seo['so']}次&nbsp;&nbsp;搜狗：{$seo['sogou']}次&nbsp;&nbsp;必应：{$seo['bing']}次&nbsp;&nbsp;搜搜：{$seo['soso']}次</p>";
            echo "<form action=\"\" method=\"get\">
					<table>
						<tr>
							<td>日期查询：&nbsp;</td>
							<td><input type=\"text\" size=\"30\" name=\"date\" value=\"{$localdate}\"/></td>
			...
			if(empty($_GET['date'])){
				...
				$result = $db -> query("select * from access_info where instr(time, '{$_GET['date']}') order by id desc limit {$offset},{$num}");
			}
            ...
            echo '<tr><th>行号</th><th>节点</th><th>用户的 IP</th><th>用户端口号</th><th>访问来源</th><th>浏览器信息</th><th>访问时间</th></tr>';
            ...
            if($currpage==1)
                ...
                echo "&nbsp;&nbsp;<a href='{$url}?page=1{$para}'>首页</a>&nbsp;&nbsp;";

            if($prev)
                echo "&nbsp;&nbsp;<a href='{$url}?page={$prev}{$para}'>上一页</a>&nbsp;&nbsp;";
            else
                echo "&nbsp;&nbsp;上一页&nbsp;&nbsp;";

            if($next)
                echo "&nbsp;&nbsp;<a href='{$url}?page={$next}{$para}'>下一页</a>&nbsp;&nbsp;";
            else
                echo "&nbsp;&nbsp;下一页&nbsp;&nbsp;";
            ...
            if($currpage==$pagenum)
                ...
                echo "&nbsp;&nbsp;<a href='{$url}?page={$pagenum}{$para}'>尾页</a>&nbsp;&nbsp;";
